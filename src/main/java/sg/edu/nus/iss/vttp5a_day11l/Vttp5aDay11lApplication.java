package sg.edu.nus.iss.vttp5a_day11l;

import java.util.Collections;

import org.springframework.boot.ApplicationArguments;
import org.springframework.boot.DefaultApplicationArguments;
import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;
import org.springframework.context.annotation.Bean;
import org.springframework.web.filter.CommonsRequestLoggingFilter;

@SpringBootApplication
public class Vttp5aDay11lApplication {

	// enable request logging
	@Bean
	public CommonsRequestLoggingFilter log() {
		CommonsRequestLoggingFilter logger = new CommonsRequestLoggingFilter();

		// set whether client address and session id should be included in log msg
		logger.setIncludeClientInfo(true);

		// set whether query string should be included in log msg
		logger.setIncludeQueryString(true);
		return logger;
	}

	public static void main(String[] args) {
		// SpringApplication.run(Vttp5aDay11lApplication.class, args);

		SpringApplication app = new SpringApplication(Vttp5aDay11lApplication.class);

		String port = "8080";

		ApplicationArguments cliOpts = new DefaultApplicationArguments(args);

		if (cliOpts.containsOption("port")) {
			port = cliOpts.getOptionValues("port").get(0);
		}

		app.setDefaultProperties(Collections.singletonMap("server.port", port));

		System.out.printf("\nApplication started on port %d\n", port);

		String dirFile = "";
		if (cliOpts.containsOption("file")) {
			dirFile = cliOpts.getOptionValues("file").get(0);

			System.out.printf("Directory file: " + dirFile);
		}
		app.run(args);
	}
}
